         ...
         if(reg_send == 1) {
             if(curr_transfer != transfer_count) { //transmiter
                 ...
                 curr_transfer = curr_transfer +  1;
                    ...
                    i2c_stop_(); //if BTF flag gets activated during sending phase, we stop
              }
         } else {
            if(curr_transfer == 0) { //we send regAddr if we haven't yet
               i2c_send(reg_addr);
               curr_transfer = curr_transfer + 1;
            } else if(BTF_I2C == 1) { //we wait for BTF flag to either do repetead start or to continue on next phase
               reg_send = 1;
               curr_transfer = 0;
               if(r_notw == 1)
                 i2c_start_();
    ...
    case RECEIVED:
         ...
         if(curr_transfer != transfer_count) {
           ...
           if(curr_transfer == transfer_count) {// || transfer_count == curr_transfer) && transfer_count != 1) {
              ACKN = 0; //prepare NACK
              i2c_stop_(); //prepare STOPPING
           }
         
           if(curr_transfer == transfer_count) {
              if(reading_xyz == 1) { //if magnet data reading state
                reading_xyz = 0;
                calcAngle(transfer); //calculate and display output
              }
           }
         } else  {
            i2c_recv(); //if we don't expect anything just dummy recv
         }
...
void init_magnetometer() {
     char creg1 = 0b10001001; // ACTIVE MODE, 2.5Hz, 32 Over Sample Ratio
     char creg2 = 0b10100000; // AUTO_RESET, RAW
     char c1;
     char c2;
     char o[5];
 ...
 void interrupt_handle() iv IVT_INT_EXTI15_10 ics ICS_AUTO {
   ...
   reading_xyz = 1; //enter magnet data reading state
   read_reg(OUT_X_MSB_ADDR, xyz, 6); //read magnet data
 }
...
//test if read and write are working
void test_rd_write() {
     char xyz[6], rd = 3;
     char o[5];
